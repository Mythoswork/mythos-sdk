...
async def test_valid_handshake_token_returns_200(client, rsa_key_pair, mock_jwks):
...
async def test_missing_lt_returns_401(client, mock_jwks):
    resp = await client.get("/.well-known/mythos-handshake")
    assert resp.status_code == 401
    assert resp.json()["error"] == "Missing launch token"


async def test_expired_token_returns_401(client, rsa_key_pair, mock_jwks):
...
async def test_wrong_purpose_returns_401(client, rsa_key_pair, mock_jwks):
...
async def test_no_purpose_claim_returns_401(client, rsa_key_pair, mock_jwks):
    payload = {
        "sub": "listing-abc",
        "exp": int(time.time()) + 120,
        "iat": int(time.time()),
...
async def test_jwks_fetch_failure_returns_503(client, rsa_key_pair):
